...
public class KdTreeStudy<T extends Clusterable> extends ClusteringAlgorithm<T> {
     ...
     */
    public KdTreeStudy(final double eps, final int minPts) {
...
//            final List<T> neighbors = getNeighbors(instance);
    ...
    private List<T> getNeighborsByRadius(final T point) {
        ...
        if (kdtree.getM_count() == 0) {
            ...
            //先批量找
            ol = kdtree.nearest(point.getFeatures(), KdTreeStudy.MIN_POINT);
        }
...
//                kdtree.delete(((T) o).getFeatures());
    ...
    public void createKd(Collection<T> instances) {
        if (instances == null || instances.isEmpty())
            throw new IllegalArgumentException("Instances does not exists!");
...
//            }
    ...
    public static void main(String[] args) {
        KdTreeStudy<Clusterable> dbscan = new KdTreeStudy<Clusterable>(KdTreeStudy.DISTANT, KdTreeStudy.MIN_POINT);
        List<Clusterable> instances = new LinkedList<Clusterable>();

        instances.add(new Instance(new double[]{1.0}, new Object[]{1, 2, 3}));
        instances.add(new Instance(new double[]{2.0}, new Object[]{1, 2, 3}));
...
//        instances.add(new Instance(new double[]{3.0, 3.0}, new Object[]{1, 2, 3}));
        ...
        dbscan.createKd(instances);

        double[] d = {1.1};
        Object[] ol = dbscan.kdtree.nearest(d, 2);
